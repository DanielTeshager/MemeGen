...
import meme
import shutil
from flask import Flask, render_template, abort, request, json
...
def meme_post():
    ...
    if image_url.split('/')[-1].split('.')[-1].lower() not in image_extensions:
        raise('Unsupported image file requested!')
    else:
